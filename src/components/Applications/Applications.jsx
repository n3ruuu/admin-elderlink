        ...
        const fetchApplications = async () => {
            try {
                ...
                const filteredApplications = response.data.filter((item) => {
                    ...
                    return (
                        item.applicationStatus.toLowerCase() === filter &&
                        validStatuses.includes(item.applicationStatus.toLowerCase())
                    )
                })
    ...
    const onStatusUpdate = async (id, newStatus) => {
        try {
            ...
            const response = await axios.put(
                `http://5.181.217.153:5000/application/members/${id}`, // Correct the URL path
